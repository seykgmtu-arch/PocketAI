...
public sealed class ImageGeneratorViewModel :
    ObservableObject,
    IDisposable
{
    ...
    private ImageGenerationMode _selectedMode =
        ImageGenerationMode.Auto;

    private IReadOnlyList<ImageLoraInfo>
        _availableLoras =
            Array.Empty<ImageLoraInfo>();

    private ImageLoraInfo? _selectedLora;
    private bool _useLora;
    private double _loraWeight = 1.0;
    ...
    public ImageGeneratorViewModel()
    {
        ...
        _engineManager =
            new ImageEngineManager(
                _baseDirectory);

        _widthText =
            _config.Images.Width.ToString(
                CultureInfo.InvariantCulture);

        _heightText =
            _config.Images.Height.ToString(
                CultureInfo.InvariantCulture);
        ...
        // Больше нет правила:
        // >640 => SDXL.
        // Реальный профиль определяется после сканирования моделей.
        _selectedProfile =
            ImageProfile.Sd15Fast512;
    ...
    public IReadOnlyList<ImageStylePreset>
        Styles { get; }

    public IReadOnlyList<ImageProfile>
        Profiles { get; }

    public IReadOnlyList<ImageGenerationMode>
        Modes { get; }
    ...
    public AsyncRelayCommand
        CheckServerCommand { get; }

    public RelayCommand
        StopCommand { get; }

    public RelayCommand
        OpenImagesFolderCommand { get; }

    public RelayCommand
        RefreshLorasCommand { get; }
    ...
    public RelayCommand
        SetLandscapeCommand { get; }

    public RelayCommand
        SetPortraitCommand { get; }
    ...
    public ImageGenerationMode SelectedMode
    {
        get => _selectedMode;
    ...
    public ImageLoraInfo? SelectedLora
    {
        get => _selectedLora;
    ...
    public async Task InitializeAsync()
    {
        ...
        {
            StatusText =
                "Определяем оборудование для image backend…";

            _hardware =
                await _hardwareProbe
                    .ProbeAsync();
            ...
            StatusText =
                "Готово. Auto использует native до 640 и Hi-Res выше 640.";
        }
        catch (Exception ex)
        {
            ErrorText =
            ...
            StatusText =
                "Не удалось определить image backend.";
        }
    }
    ...
    private async Task CheckServerAsync()
    {
        ...
        {
            StatusText =
                "Проверка image backend остановлена.";
        }
        catch (Exception ex)
        {
            ErrorText =
    ...
    private async Task GenerateAsync()
    {
        ...
        try
        {
            ...
            var finalNegativePrompt =
                Combine(
                    NegativePrompt.Trim(),
                    style.NegativeSuffix);

            IReadOnlyList<ImageLoraSelection>
                loras =
                    Array.Empty<ImageLoraSelection>();

            if (UseLora &&
                SelectedLora is not null)
            {
                loras =
                    new[]
                    {
                        new ImageLoraSelection(
                            SelectedLora.ApiPath,
                            LoraWeight)
                    };
            }
            ...
            StatusText =
                highResExpected
                    ? $"Генерация {size.Width}×{size.Height} · Hi-Res · {session.Backend}…"
                    : $"Генерация {size.Width}×{size.Height} · Native · {session.Backend}…";
            ...
            StatusText =
                ...
                $"{result.FilePaths.Count} image(s) · " +
                $"{(result.UsedHighRes ? "Hi-Res" : "Native")}. " +
                result.SizeMessage;
        }
        catch (OperationCanceledException)
        {
    ...
    private ImageSizeValidationResult
        GetNormalizedSize()
    {
        var requestedWidth =
            ParseInt(
